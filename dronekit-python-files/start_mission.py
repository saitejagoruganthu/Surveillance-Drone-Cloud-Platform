...
def fetch_mission_details_from_database(mission_id):
    ...
    url = "http://{}:5001/api/getonemission1/{}".format(IP_ADDRESS, mission_id)
...
waypoint_0_long = mission_details[0]["mission_waypoints"][0]['longitude']

# sitl = None
# Constructing the home argument using the waypoint coordinates
home_arg = '--home={},{},0,270'.format(waypoint_0_lat, waypoint_0_long)

# Original SITL arguments
sitl_args = [];
sitl_args.append('-' + inst_num)
sitl_args.append('--model')
sitl_args.append('quad')

# Adding the home argument to the sitl_args array
sitl_args.append(home_arg)
print(sitl_args)
# Downloading and launching SITL with the updated arguments
sitl = SITL()
...
def send_telemetry(data):
    url = 'http://{}:5001/api/telemetry/{}'.format(IP_ADDRESS, mission_id)
...
def send_notification(data):
    url = 'http://{}:5001/api/notification/{}'.format(IP_ADDRESS, mission_id)
    headers = {'Content-Type': 'application/json'}
    response = requests.post(url, data=json.dumps(data, default=json_util.default), headers=headers, verify=False)
    if response.status_code == 200:
